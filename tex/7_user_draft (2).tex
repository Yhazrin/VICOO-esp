...
% 本文件位于仓库 \texttt{tex/}；配图在同目录 \texttt{user-manual-figures/}（Playwright 生成）。需 API + 消费者站点（9111）；U3 需 admin（\texttt{cd admin \&\& npm run dev}，部分环境为 \texttt{http://localhost:5173}）。
...
\IfFileExists{user-manual-figures/u1-impact-shop.png}{%
\begin{figure}[H]\centering\includegraphics[width=0.94\linewidth]{user-manual-figures/u1-impact-shop.png}\caption*{Figure U1: Impact shop (\texttt{/impact/shop}).}\end{figure}%
}{\placeholder{Generate \texttt{user-manual-figures/u1-impact-shop.png} via \texttt{npm run screenshots:manual}.}}
...
\begin{figure}[H]\centering\includegraphics[width=0.94\linewidth]{user-manual-figures/u2-globe-timeline.png}\caption*{Figure U2: Traceability globe and timeline (\texttt{/impact/shop/1}).}\end{figure}%
...
\begin{figure}[H]\centering\includegraphics[width=0.94\linewidth]{user-manual-figures/u3-admin-products.png}\caption*{Figure U3: Admin console --- Products (\texttt{/admin/products}).}\end{figure}%
