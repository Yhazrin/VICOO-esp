...
export interface DonateOrdersHandle {
  scrollTo: () => void;
}

const DonateOrders = forwardRef<DonateOrdersHandle>(function DonateOrders(_props, ref) {
    ...
    <div ref={sectionRef}>
      <SectionContainer decorativeDivider>
        <h2 className="font-display text-h2 text-ink mb-2">
          {t('donateClothing.ordersTitle', 'My Recycling Orders')}
        </h2>
        <p className="font-body text-body text-ink-faded mb-10">
          {t('donateClothing.ordersSubtitle', 'Track recycling progress, review outcomes, and linked resale items here.')}
        </p>
        ...
        {!isAuthenticated ? (
          <motion.div {...fadeUp()} className="rounded-xl border border-rust/20 bg-aged-stock/30 p-8 text-center max-w-lg mx-auto">
            ...
            <Link to="/login" className="font-body text-caption text-rust tracking-[0.1em] uppercase hover:underline">
              {t('donateClothing.goToLogin', 'Go to Login')} &rarr;
            </Link>
          </motion.div>
        ) : (
          <>
            ...
            <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 md:flex-col md:items-end md:gap-2 pl-3 md:pl-0">
                    ...
                    {order.status === 'listed' && order.productLink && (
                      <Link to={order.productLink} className="font-body text-caption text-rust tracking-[0.1em] uppercase hover:underline">
                        {t('donateClothing.viewProduct', 'View Product')} &rarr;
                      </Link>
                    )}
              ...
              {!isLoading && filteredOrders.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <a href="#donate-form" className="font-body text-caption text-rust tracking-[0.1em] uppercase hover:underline">
                    {t('donateClothing.submitFirst', 'Submit your first recycling request')} &rarr;
                  </a>
                </div>
              )}
            </div>
