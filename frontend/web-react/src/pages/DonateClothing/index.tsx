...
import DonateOrders, { type DonateOrdersHandle } from './components/DonateOrders';
import DonateShowcase from './components/DonateShowcase';

export default function DonateClothing() {
  const { t } = useTranslation();
  const prefersReducedMotion = useReducedMotion();
  const ordersRef = useRef<DonateOrdersHandle>(null);
    ...
    <PageWrapper>
      ...
      <div className="bg-aged-stock/40 border-b border-rust/20 px-4 py-2 flex items-center justify-center gap-2">
        ...
        <span className="font-body text-caption text-sepia-mid">
          {t('donateClothing.disclaimer', 'Product cards are sample content. Signed-in users will see real recycling requests and synced progress.')}
        </span>
      </div>
      ...
      <SectionContainer noTopSpacing>
        <div className="pt-12 md:pt-16 pb-6">
          ...
          >
            {t('donateClothing.heroTitle', 'Donate Clothing')}
          </motion.h1>
          <motion.p
          ...
          >
            {t('donateClothing.heroSubtitle', 'Give old garments a second life, from submission and review to relisting and reuse.')}
          </motion.p>
          <div className="flex flex-wrap gap-4 md:gap-6">
            {[
              { value: '12,000+', label: t('donateClothing.statRecycled', 'Garments Recovered') },
              { value: '8,500 kg', label: t('donateClothing.statCarbon', 'CO₂ Reduced') },
              { value: '87%', label: t('donateClothing.statRate', 'Reuse Rate') },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
      ...
      {/* Form section */}
      <section id="donate-form" className="bg-aged-stock/30 py-16 md:py-24 relative">
        <SectionContainer noTopSpacing>
          <motion.h2
            initial={prefersReducedMotion ? false : { opacity: 0, y: 16 }}
          ...
          >
            {t('donateClothing.submitTitle', 'Submit Old Clothes')}
          </motion.h2>
          <motion.p
            initial={prefersReducedMotion ? false : { opacity: 0, y: 12 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            {t('donateClothing.submitSubtitle', 'Fill in the form below to start a recycling request. Our operations team reviews submissions within 1-3 business days.')}
          </motion.p>
        </SectionContainer>
        <DonateForm onSubmitted={() => ordersRef.current?.scrollTo()} />
      </section>

      <MagazineDivider variant="decorative" />

      {/* Orders */}
      <DonateOrders ref={ordersRef} />
